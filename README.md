...
Versions are derived from git tags by [MinVer](https://github.com/adamralph/minver) — tag a commit `v8.0.0` and it becomes version `8.0.0`; untagged commits after that get `8.0.1-alpha.0.N`. Pushing a `v*` tag triggers the [`release.yml`](.github/workflows/release.yml) workflow, which builds, tests, packs all 8 packages (+ `.snupkg` symbol packages with SourceLink), and pushes to NuGet.org. See [Contributing — Releasing](docs/site/develop/contributing.md#releasing) for the full process and prerequisites.
