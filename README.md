...
The SQL Server, PostgreSQL, MySQL, and Oracle integration suites provision their engine with **[Testcontainers](https://dotnet.testcontainers.org/)** — the only host dependency is **Docker**. When Docker is unavailable every live fact **skips** (via `SkippableFact`) rather than failing, so `dotnet test` stays green without Docker. CI runs PostgreSQL, MySQL, SQL Server, and Oracle on every PR. See [Project status](docs/site/develop/project-status.md) for the current state and the [Roadmap](docs/site/develop/roadmap.md) for what's next.
