...
/// <summary>
/// End-to-end CRUD against composite-key entities on PostgreSQL. Uses a minimal DDL without
...
public sealed class CompositeKeyIntegrationTests
{
    ...
    /// Column types mirror <see cref="Inquiry.Northwind.NorthwindSchema.PostgreSqlDdl"/>.
    /// </summary>
    ...
    public CompositeKeyIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SelectByKeyFindsRowMatchingBothKeyColumns()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_sel");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_null");
        var store = harness.GetRequiredService<OrderDetailStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_upd");
        var store = harness.GetRequiredService<OrderDetailStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_del");
        var store = harness.GetRequiredService<OrderDetailStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_order");
        var store = harness.GetRequiredService<OrderDetailStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_dup");
        var store = harness.GetRequiredService<OrderDetailStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "comp_str");
