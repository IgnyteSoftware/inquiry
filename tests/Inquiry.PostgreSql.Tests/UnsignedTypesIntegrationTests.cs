...
///   sbyte -1            → byte 255             → SMALLINT (PostgreSQL has no TINYINT)
///   ushort 40000        → short -25536         → SMALLINT
///   uint 3_000_000_000  → int -1_294_967_296   → INTEGER
...
public sealed class UnsignedTypesIntegrationTests
{
    ...
    public UnsignedTypesIntegrationTests(PostgreSqlContainerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task Byte_RoundTrips()
    {
        ...
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgByte");
    ...
    public async Task Int16_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgI16");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task Int32_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgI32");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task Int64_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgI64");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task EnumInt32_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgEI32");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task SByte_Low_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgSB2");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task SByte_NegativeOne_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgSB3");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // sbyte -1 → unchecked((byte)-1) = 255 → SMALLINT; reads back GetInt16=255 → (sbyte)=-1.
    ...
    public async Task SByte_MinValue_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgSB4");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // sbyte.MinValue (-128) → unchecked((byte)) = 128 → SMALLINT; reads back to -128.
    ...
    public async Task UInt16_Low_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU162");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task UInt16_AboveShortMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU163");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task UInt16_MaxValue_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU164");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task UInt32_Low_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU322");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task UInt32_AboveIntMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU323");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // 3_000_000_000 > int.MaxValue (2_147_483_647); stored as -1_294_967_296 in INTEGER, reads back exactly.
    ...
    public async Task UInt32_MaxValue_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU324");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // uint.MaxValue → int -1 → INTEGER; reads back to uint.MaxValue.
    ...
    public async Task UInt64_Low_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU642");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task UInt64_AboveLongMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgU643");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task EnumSByte_Negative_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgESBn");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // SampleEnumSByte.Negative = -1 → byte 255 → SMALLINT; reads back to the enum member.
    ...
    public async Task EnumUInt16_AboveShortMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgEU16a");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
    ...
    public async Task EnumUInt32_AboveIntMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgEU32a");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();

        // SampleEnumUInt32.AboveIntMax = 3_000_000_000 (> int.MaxValue) → int -1_294_967_296 → INTEGER.
    ...
    public async Task EnumUInt64_AboveLongMax_RoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.UnsignedTypesPostgreSqlDdl, "UnsignedPgEU64a");
        var store = harness.GetRequiredService<UnsignedTypesItemStore>();
