...
public sealed class TopByOrderIntegrationTests
{
    ...
    public TopByOrderIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = """CREATE TABLE "TopSale" ("Id" BIGSERIAL PRIMARY KEY, "Amount" DECIMAL NOT NULL, "Region" TEXT NOT NULL);""";

    [SkippableFact]
    public async Task GetCheapestReturnsLowestAmountRow()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "toporder");
