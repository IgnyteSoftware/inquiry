...
{
    private const string Ddl = """CREATE TABLE "SeqDoc" ("Id" UUID NOT NULL PRIMARY KEY, "Title" TEXT NOT NULL);""";

    private readonly PostgreSqlContainerFixture _fixture;
    public SequentialGuidIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task UnsetKeyIsAssignedAndRoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "seqguid");
