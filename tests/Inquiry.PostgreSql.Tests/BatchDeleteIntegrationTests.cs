...
/// <summary>Collection predicate delete end-to-end against PostgreSQL: DELETE ... WHERE Id IN (...) removes exactly the
/// listed rows; an empty collection is a no-op.</summary>
[Collection(PostgreSqlCollection.Name)]
public sealed class BatchDeleteIntegrationTests
{
    private const string Ddl = @"CREATE TABLE ""Gadget"" (""Id"" BIGSERIAL PRIMARY KEY, ""Name"" TEXT NOT NULL);";

    private readonly PostgreSqlContainerFixture _fixture;
    public BatchDeleteIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    private async Task<PostgreSqlTestHarness> SeedAsync()
    {
        var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchdel");
