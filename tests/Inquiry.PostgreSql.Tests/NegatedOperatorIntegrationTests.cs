...
public sealed class NegatedOperatorIntegrationTests
{
    ...
    public NegatedOperatorIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = """CREATE TABLE "NegatedProduct" ("Id" BIGSERIAL PRIMARY KEY, "Name" TEXT NOT NULL, "Qty" INTEGER NOT NULL);""";

    [SkippableFact]
    public async Task NotLikeExcludesPatternMatches()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "negated");
