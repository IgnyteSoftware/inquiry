...
/// <summary>
/// W6 optimistic concurrency against real PostgreSQL via the shared <see cref="VersionedItem"/> catalog
...
public sealed class ConcurrencyIntegrationTests
{
    ...
    public ConcurrencyIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SuccessfulUpdateBumpsVersion()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.PostgreSqlDdl, "conc");
