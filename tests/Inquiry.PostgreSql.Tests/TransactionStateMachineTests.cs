...
/// <summary>
/// Mirror of <c>Inquiry.Sqlite.Tests.TransactionStateMachineTests</c> for PostgreSQL.
...
/// PostgreSQL wrapper would duplicate ~150 lines of Npgsql-specific plumbing to test the same
...
public sealed class TransactionStateMachineTests
{
    ...
    public TransactionStateMachineTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task ConcurrentTransactionsOnSameDIScopeBothCommit()
    {
        ...
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "tx_concurrent_both");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "tx_concurrent_mixed");
        var inquiry = harness.GetRequiredService<IInquiry>();
        var store = harness.GetRequiredService<CustomerStore>();
    ...
    public async Task ManyConcurrentTransactionsAllCommitIndependently()
    {
        ...
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "tx_concurrent_many");
        var inquiry = harness.GetRequiredService<IInquiry>();
        var store = harness.GetRequiredService<CustomerStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "tx_custom_commit");
        var inquiry = harness.GetRequiredService<IInquiry>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "tx_custom_rollback");
        var inquiry = harness.GetRequiredService<IInquiry>();
        var customers = harness.GetRequiredService<CustomerStore>();
        var atomic = new CustomerAtomicService(inquiry, customers);
...
/// transactions feature page describes, ported to the PostgreSQL live engine.
/// </summary>
