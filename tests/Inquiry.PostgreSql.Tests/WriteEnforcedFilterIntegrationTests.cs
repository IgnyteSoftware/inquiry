...
/// PostgreSQL. Three things only a live engine settles: that a cross-tenant key write affects nothing
...
public sealed class WriteEnforcedFilterIntegrationTests
{
    ...
    public WriteEnforcedFilterIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;
    ...
    private Task<PostgreSqlTestHarness> CreateHarnessAsync()
        => PostgreSqlTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.WriteEnforcedDocPostgreSqlDdl, "wenf");
    ...
    public async Task OwnRowUpdateReturningSurvivesANoOpAndAFilterColumnChange()
    {
        ...
        using (Scope(1))
        {
            // PostgreSQL returns the row natively via UPDATE ... RETURNING, so these cases are less
            // fragile than on the emulated-returning dialects — but the same contract holds: a no-op
            // update must still come back with the row, not read as a miss.
