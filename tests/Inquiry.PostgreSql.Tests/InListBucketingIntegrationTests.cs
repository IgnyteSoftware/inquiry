...
/// <summary>
/// Live coverage of IN-list bucketing and its NOT IN counterpart against real PostgreSQL: the expansion
/// pads each list to the next power of two by repeating an element, and these round-trips prove the padded
/// SQL returns the correct rows on a real engine across bucket boundaries.
/// </summary>
...
public sealed class InListBucketingIntegrationTests
{
    ...
    {
        var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, ProductsDdl, "inbucket");
