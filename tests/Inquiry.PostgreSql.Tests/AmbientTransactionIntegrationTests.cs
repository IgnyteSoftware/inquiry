...
public sealed class AmbientTransactionIntegrationTests
{
    ...
    public AmbientTransactionIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task StoreCallInsideTransactionIsRolledBack()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "Ambient");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harnessA = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientA");
        await using var harnessB = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientB");
