...
public sealed class CancellationTokenPropagationTests
{
    ...
    public async Task InquiryPipelineCancellationStopsInFlightOperationPromptly()
    {
        ...
        PostgreSqlTestHarness? harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancel");

        using var cts = new CancellationTokenSource(TimeSpan.FromMilliseconds(500));
        try
        {
            var inquiry = harness.GetRequiredService<IInquiry>();
            var execution = inquiry.ExecuteAsync($"SELECT pg_sleep(30)", cts.Token);
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelSelect");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelInsert");
        var store = harness.GetRequiredService<CustomerStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelStream");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelScalar");
        var inquiry = harness.GetRequiredService<IInquiry>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelTx");
        var inquiry = harness.GetRequiredService<IInquiry>();
    ...
    private static async Task ObserveAndDisposeAsync(Task execution, PostgreSqlTestHarness harness)
    {
        try
        {
