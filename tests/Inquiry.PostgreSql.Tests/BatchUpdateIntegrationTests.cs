...
/// <summary>Batch update end-to-end against PostgreSQL: one UPDATE per row in a single command updates
/// each row by key; an empty collection is a no-op.</summary>
[Collection(PostgreSqlCollection.Name)]
public sealed class BatchUpdateIntegrationTests
{
    private const string Ddl = @"CREATE TABLE ""Sku"" (""Id"" BIGSERIAL PRIMARY KEY, ""Name"" TEXT NOT NULL, ""Price"" NUMERIC NOT NULL);";

    private readonly PostgreSqlContainerFixture _fixture;
    public BatchUpdateIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    private async Task<PostgreSqlTestHarness> SeedAsync()
    {
        var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchupd");
