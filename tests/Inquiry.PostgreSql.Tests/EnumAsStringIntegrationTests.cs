...
public sealed class EnumAsStringIntegrationTests
{
    ...
    public EnumAsStringIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = """CREATE TABLE "Ticket" ("Id" BIGSERIAL PRIMARY KEY, "Status" TEXT NOT NULL, "Prior" TEXT NULL);""";

    [SkippableFact]
    public async Task StoresAndReadsEnumMemberNames()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "enumstr");
