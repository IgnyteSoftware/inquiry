...
public sealed class EagerLoadingIntegrationTests
{
    ...
    public EagerLoadingIntegrationTests(PostgreSqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task SelectByKeyEagerLeavesParentNullForOrphanForeignKey()
    {
        ...
        await using var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, OrphanTerritoryDdl, "eager");
        var territoryStore = harness.GetRequiredService<TerritoryStore>();
    ...
    private async Task<(PostgreSqlTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)> CreateGridHarnessAsync()
    {
        var probe = new BatchExecutionProbe();
        var recorder = new RecordingCommandInterceptor();
        var harness = await PostgreSqlTestHarness.CreateFromDdlAsync(
