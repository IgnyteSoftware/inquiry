...
/// <summary>
/// Derived query methods end-to-end against SQLite: a field-less <c>[InquirySelectAllByField]</c>
/// filters on columns inferred from the method name.
/// </summary>
public sealed class DerivedQueryIntegrationTests
{
    private const string Ddl = "CREATE TABLE Person (Id INTEGER PRIMARY KEY AUTOINCREMENT, Country TEXT NOT NULL, City TEXT NOT NULL);";

    [Fact]
    public async Task DerivedSingleAndMultiFieldFiltersWork()
    {
        await using var harness = await SqliteTestHarness.CreateAsync(Ddl, "Derived");
