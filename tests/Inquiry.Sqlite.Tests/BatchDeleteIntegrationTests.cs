...
{
    private const string Ddl = "CREATE TABLE Gadget (Id INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT NOT NULL);";

    private static async Task<SqliteTestHarness> SeedAsync()
    {
        var harness = await SqliteTestHarness.CreateAsync(Ddl, "BatchDelete");
