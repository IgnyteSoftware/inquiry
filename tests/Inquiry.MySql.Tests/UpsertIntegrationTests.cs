...
public sealed class UpsertIntegrationTests
{
    ...
    public UpsertIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task UpsertInsertsWhenRowDoesNotExist()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "upsert_ins");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "upsert_upd");
        var store = harness.GetRequiredService<CustomerStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "upsert_conc");
        var store = harness.GetRequiredService<CustomerStore>();
