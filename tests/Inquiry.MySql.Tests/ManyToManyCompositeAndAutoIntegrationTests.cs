...
/// <summary>
/// End-to-end coverage for the two many-to-many extensions in #80 against real MySql: a related entity
...
public sealed class ManyToManyCompositeAndAutoIntegrationTests
{
    ...
    public ManyToManyCompositeAndAutoIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;
    private static async Task<(MySqlTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)>
        CreateGridHarnessAsync(string adminConnectionString, string ddl, string name)
    {
        ...
        var harness = await MySqlTestHarness.CreateFromDdlAsync(adminConnectionString, ddl, name,
            configureServices: s =>
            {
    ...
    private Task<MySqlTestHarness> CompositeHarnessAsync()
        => MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.CompositeManyToManyMySqlDdl, "M2MComposite");

    private Task<MySqlTestHarness> AutoHarnessAsync()
        => MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.AutoJunctionMySqlDdl, "M2MAuto");
