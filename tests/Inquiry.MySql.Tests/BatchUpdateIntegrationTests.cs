...
/// <summary>Batch update end-to-end against MySQL: one UPDATE per row in a single command updates
/// each row by key; an empty collection is a no-op.</summary>
[Collection(MySqlCollection.Name)]
public sealed class BatchUpdateIntegrationTests
{
    private const string Ddl = "CREATE TABLE `Sku` (`Id` BIGINT AUTO_INCREMENT PRIMARY KEY, `Name` VARCHAR(255) NOT NULL, `Price` DECIMAL(18,2) NOT NULL);";

    private readonly MySqlContainerFixture _fixture;
    public BatchUpdateIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    private async Task<MySqlTestHarness> SeedAsync()
    {
        var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchupd");
