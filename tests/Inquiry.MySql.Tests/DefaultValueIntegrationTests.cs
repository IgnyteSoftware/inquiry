...
public sealed class DefaultValueIntegrationTests
{
    ...
    public async Task InsertReturningUsesDatabaseDefaultForDefaultedColumn()
    {
        ...
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, DefaultedItemDdl, "defcol");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, DefaultedKeyItemDdl, "defkey");
