...
public sealed class AmbientTransactionIntegrationTests
{
    ...
    public AmbientTransactionIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task StoreCallInsideTransactionIsRolledBack()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "Ambient");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harnessA = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientA");
        await using var harnessB = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientB");
