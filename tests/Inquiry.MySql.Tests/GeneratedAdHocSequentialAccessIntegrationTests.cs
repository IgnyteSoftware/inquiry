...
[InquiryAdHoc]
public sealed class MySqlSequentialAdHocRow
{
...
public sealed class GeneratedAdHocSequentialAccessIntegrationTests
{
    ...
    public GeneratedAdHocSequentialAccessIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task GeneratedAdHocWideRowsStreamAndReleaseResources()
    {
        ...
        var buffered = await inquiry.QueryListAsync<MySqlSequentialAdHocRow>(new Inquiry.Commands.InquiryCommand(SelectSql));
        AssertRows(buffered);

        var streamed = new List<MySqlSequentialAdHocRow>();
        await foreach (var row in inquiry.QueryAsync<MySqlSequentialAdHocRow>(new Inquiry.Commands.InquiryCommand(SelectSql)))
        {
            streamed.Add(row);
        }
        AssertRows(streamed);
        ...
        await foreach (var row in transaction.QueryAsync<MySqlSequentialAdHocRow>(new Inquiry.Commands.InquiryCommand(SelectSql)))
        {
            AssertRow(row, 1);
            break;
        }
    ...
    private static void AddParameters(MySqlCommand command, int id)
    {
    ...
    private static void AssertRows(IReadOnlyList<MySqlSequentialAdHocRow> rows)
    {
        Assert.Equal(RowCount, rows.Count);
        for (var i = 0; i < rows.Count; i++) AssertRow(rows[i], i + 1);
    }

    private static void AssertRow(MySqlSequentialAdHocRow row, int id)
    {
