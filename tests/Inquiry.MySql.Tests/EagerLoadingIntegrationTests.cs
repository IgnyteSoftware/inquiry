...
public sealed class EagerLoadingIntegrationTests
{
    ...
    public EagerLoadingIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task SelectByKeyEagerLeavesParentNullForOrphanForeignKey()
    {
        ...
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, OrphanTerritoryDdl, "eager");
        var territoryStore = harness.GetRequiredService<TerritoryStore>();
    ...
    private async Task<(MySqlTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)> CreateGridHarnessAsync()
    {
        var probe = new BatchExecutionProbe();
        var recorder = new RecordingCommandInterceptor();
        var harness = await MySqlTestHarness.CreateFromDdlAsync(
