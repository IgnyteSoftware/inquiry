...
/// <summary>
/// W6 optimistic concurrency against real MySQL via the shared <see cref="VersionedItem"/> catalog
...
public sealed class ConcurrencyIntegrationTests
{
    ...
    public ConcurrencyIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SuccessfulUpdateBumpsVersion()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "conc");
