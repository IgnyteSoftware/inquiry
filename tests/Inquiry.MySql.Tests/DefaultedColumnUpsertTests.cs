...
/// during upsert against live MySQL:
...
/// SQLite dialect. This test exists because MySQL's <c>ON DUPLICATE KEY UPDATE col = VALUES(col)</c>
...
public sealed class DefaultedColumnUpsertTests
{
    ...
    public DefaultedColumnUpsertTests(MySqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task UpsertUpdateBranchPersistsEntityStatusInsteadOfReverttingToDatabaseDefault()
    {
        ...
        // Under the pre-fix MySQL builder, this branch assigned `Status = VALUES(Status)`
