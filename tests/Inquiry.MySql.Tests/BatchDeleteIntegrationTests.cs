...
/// <summary>Collection predicate delete end-to-end against MySQL: DELETE ... WHERE Id IN (...) removes exactly the
...
public sealed class BatchDeleteIntegrationTests
{
    ...
    public BatchDeleteIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    private async Task<MySqlTestHarness> SeedAsync()
    {
        var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchdel");
