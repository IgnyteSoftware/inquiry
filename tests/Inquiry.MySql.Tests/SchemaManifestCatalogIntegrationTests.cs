...
public sealed class SchemaManifestCatalogIntegrationTests
{
    ...
    public SchemaManifestCatalogIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task GeneratedManifestMatchesLiveCatalog()
    {
        ...
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, ddl, "manifest");
        await using var connection = new MySqlConnection(harness.ConnectionString); await connection.OpenAsync();
        await AssertCatalogAsync(connection);
    }

    private static async Task AssertCatalogAsync(MySqlConnection connection)
    {
