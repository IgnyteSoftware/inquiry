...
/// <summary>
/// Pins the concurrent-upsert contract on MySQL (audit P2 #6). The client-supplied-key path uses
/// single-statement <c>INSERT … ON DUPLICATE KEY UPDATE</c>, which MySQL executes atomically — N
...
public sealed class UpsertConcurrencyTests
{
    ...
    public UpsertConcurrencyTests(MySqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task ConcurrentUpsertsOfSameKeyEndInOneRowMatchingOneInput()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateAsync(_fixture.AdminConnectionString, "upsert_concurrent");
