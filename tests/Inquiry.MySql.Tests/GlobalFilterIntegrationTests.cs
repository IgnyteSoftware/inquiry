...
/// <summary>
/// Global query filter against real MySQL via the shared <see cref="GlobalFilterDoc"/> and
...
public sealed class GlobalFilterIntegrationTests
{
    ...
    public GlobalFilterIntegrationTests(MySqlContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task GlobalFilterHidesUnpublishedRowsFromEverySelect()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MySqlTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.GlobalFilterMySqlDdl, "gf");
