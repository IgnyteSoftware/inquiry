...
/// <summary>Batch update end-to-end against Oracle: one UPDATE per row in a single command updates
/// each row by key; an empty collection is a no-op.</summary>
[Collection(OracleCollection.Name)]
public sealed class BatchUpdateIntegrationTests
{
    private const string Ddl = "CREATE TABLE Sku (Id NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, Name VARCHAR2(255) NOT NULL, Price NUMBER(18,2) NOT NULL)";

    private readonly OracleContainerFixture _fixture;
    public BatchUpdateIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;

    private async Task<OracleTestHarness> SeedAsync()
    {
        var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchupd");
