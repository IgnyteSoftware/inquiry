...
public sealed class EagerLoadingIntegrationTests
{
    ...
    public EagerLoadingIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;

    /// <summary>Region + Territories DDL without FK constraints for orphan-FK tests.</summary>
    private const string OrphanTerritoryDdl = """
        CREATE TABLE Region (
            RegionID           NUMBER(10) NOT NULL PRIMARY KEY,
            RegionDescription  VARCHAR2(255) NOT NULL
        );
        CREATE TABLE Territories (
            TerritoryID           VARCHAR2(40) NOT NULL PRIMARY KEY,
    ...
    public async Task SelectByKeyEagerLeavesParentNullForOrphanForeignKey()
    {
        ...
        await using var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, OrphanTerritoryDdl, "eager");
        var territoryStore = harness.GetRequiredService<TerritoryStore>();
    ...
    private async Task<(OracleTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)> CreateGridHarnessAsync()
    {
        var probe = new BatchExecutionProbe();
        var recorder = new RecordingCommandInterceptor();
        var harness = await OracleTestHarness.CreateAsync(_fixture.AdminConnectionString, "eager",
