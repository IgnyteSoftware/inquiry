...
/// <summary>
/// End-to-end coverage for the two many-to-many extensions in #80 against real Oracle: a related entity
...
public sealed class ManyToManyCompositeAndAutoIntegrationTests
{
    ...
    public ManyToManyCompositeAndAutoIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;
    private static async Task<(OracleTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)>
        CreateGridHarnessAsync(string adminConnectionString, string ddl, string name)
    {
        ...
        var harness = await OracleTestHarness.CreateFromDdlAsync(adminConnectionString, ddl, name,
            configureServices: s =>
            {
    ...
    private Task<OracleTestHarness> CompositeHarnessAsync()
        => OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.CompositeManyToManyOracleDdl, "M2MComposite");

    private Task<OracleTestHarness> AutoHarnessAsync()
        => OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.AutoJunctionOracleDdl, "M2MAuto");
    ...
    public async Task CompositeKeyEagerLoadCostsOneRoundTrip()
    {
        ...
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.CompositeManyToManyOracleDdl, "M2MCompositeGrid");
    ...
    public async Task CompositeKeyAllEagerCostsOneRoundTrip()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.CompositeManyToManyOracleDdl, "M2MCompositeAllGrid");
        await using var _ = harness;
    ...
    public async Task AutoJunctionEagerLoadCostsOneRoundTrip()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.AutoJunctionOracleDdl, "M2MAutoGrid");
        await using var _ = harness;
