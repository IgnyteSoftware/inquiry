...
/// <summary>
/// W6 optimistic concurrency against real Oracle via the shared <see cref="VersionedItem"/> catalog
...
public sealed class ConcurrencyIntegrationTests
{
    ...
    public ConcurrencyIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SuccessfulUpdateBumpsVersion()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.OracleDdl, "conc");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.OracleDdl, "concthrow");

        await using var throwing = new ServiceCollection()
            .AddInquiry(o => o.ThrowOnConcurrencyConflict = true)
            .AddInquiryOracle(harness.ConnectionString)
