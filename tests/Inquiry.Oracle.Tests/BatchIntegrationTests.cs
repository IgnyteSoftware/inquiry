...
/// Oracle: <c>InsertAll</c> inserts a collection in one statement, <c>UpdateAll</c> updates each row
...
public sealed class BatchIntegrationTests
{
    ...
    public BatchIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await OracleTestHarness.CreateAsync(_fixture.AdminConnectionString, "batchempty");
        var regions = harness.GetRequiredService<RegionStore>();
