...
/// Oracle. Three things only a live engine settles: that a cross-tenant key write affects nothing
...
public sealed class WriteEnforcedFilterIntegrationTests
{
    ...
    public WriteEnforcedFilterIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;
    ...
    private Task<OracleTestHarness> CreateHarnessAsync()
        => OracleTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, FeatureSchema.WriteEnforcedDocOracleDdl, "wenf");
