...
/// <summary>
/// Global query filter against real Oracle via the shared <see cref="GlobalFilterDoc"/> and
...
public sealed class GlobalFilterIntegrationTests
{
    ...
    public GlobalFilterIntegrationTests(OracleContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task GlobalFilterHidesUnpublishedRowsFromEverySelect()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.GlobalFilterOracleDdl, "gf");
