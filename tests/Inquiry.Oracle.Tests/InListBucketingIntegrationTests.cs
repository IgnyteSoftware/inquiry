...
/// <summary>
/// Live coverage of IN-list bucketing and its NOT IN counterpart against real Oracle: the expansion pads
...
public sealed class InListBucketingIntegrationTests
{
    ...
    private const string ProductsDdl = """
        CREATE TABLE Products (ProductID NUMBER(10) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, ProductName VARCHAR2(255) NOT NULL, UnitPrice NUMBER(18,2), UnitsInStock NUMBER(5), CategoryID NUMBER(10) NULL, Discontinued NUMBER(1) DEFAULT 0 NOT NULL)
        """;

    private async Task<OracleTestHarness> SeedAsync()
    {
        var harness = await OracleTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, ProductsDdl, "inbucket");
