...
public sealed partial class InquiryGeneratorTests
{
    ...
    private static string SetterProjectionSource(string titleAccessors) => $$"""
        using System.Collections.Generic;
        using System.Threading;
        using System.Threading.Tasks;
        using Inquiry;
        ...
        [InquiryProjection(typeof(Doc))]
        public sealed class DocTitle
        {
            [InquiryColumn("Id")]
            public long Id { get; set; }

            [InquiryColumn("Title")]
            public string Title { get; {{titleAccessors}} } = string.Empty;
        }
        """;
    ...
    public void ProjectionPropertyWithoutAccessibleSetterReportsINQ009(string titleAccessors)
    {
        // The projection materializer assigns via an object initializer, so a get-only or
        // private-setter property is unusable. Before this the author saw only a raw CS0200/CS0272
        // pointing into generated code.
        var result = RunGenerator(SetterProjectionSource(titleAccessors));

        Assert.Contains(
            result.RunResult.Diagnostics,
            static d => d.Id == "INQ009" && d.Severity == DiagnosticSeverity.Error && d.GetMessage().Contains("DocTitle.Title", StringComparison.Ordinal));
    }

    [Fact]
    public void ProjectionPropertyWithInitSetterReportsNoINQ009()
    {
        var result = RunGenerator(SetterProjectionSource("init;"));

        AssertNoErrors(result);
        Assert.DoesNotContain(result.RunResult.Diagnostics, static d => d.Id == "INQ009");
    }
}
