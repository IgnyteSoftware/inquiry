...
/// <summary>
/// End-to-end test of multi-field select on MariaDB. Uses a minimal DDL without foreign keys
...
public sealed class MultiFieldSelectIntegrationTests
{
    ...
    public MultiFieldSelectIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SelectByTwoFieldsReturnsOnlyRowsMatchingBoth()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "mf_both");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "mf_empty");
        var orders = harness.GetRequiredService<OrderStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "mf_single");
        var orders = harness.GetRequiredService<OrderStore>();
