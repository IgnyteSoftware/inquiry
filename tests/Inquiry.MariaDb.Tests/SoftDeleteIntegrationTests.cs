...
/// <summary>
/// Soft delete against real MariaDB via the shared <see cref="SoftItem"/> catalog entity: a soft
...
public sealed class SoftDeleteIntegrationTests
{
    ...
    public SoftDeleteIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    private async Task<(MariaDbTestHarness Harness, SoftItemStore Store, long Id)> SeedOneAsync()
    {
        var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "soft");
