...
{
    private readonly MariaDbContainerFixture _f;

    public SchemaPrimitiveIntegrationTests(MariaDbContainerFixture f) => _f = f;
    ...
    public async Task GeneratedSchemaPrimitivesAreCatalogedAndEnforced()
    {
        ...
        await using var h = await MariaDbTestHarness.CreateFromDdlAsync(_f.AdminConnectionString, d, "genprimitive");
        ...
        Assert.Equal(2, await S(c, "SELECT COUNT(*) FROM information_schema.table_constraints WHERE constraint_schema=DATABASE() AND table_name='PrimitiveChild' AND constraint_name IN ('CK_PrimitiveChild_Quantity','CK_PrimitiveChild_Code') AND constraint_type='CHECK'"));
