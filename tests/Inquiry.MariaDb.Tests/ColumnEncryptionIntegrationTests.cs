...
public sealed class ColumnEncryptionIntegrationTests
{
    ...
    private const string Ddl =
        "CREATE TABLE `SecretRecord` (`Id` BIGINT AUTO_INCREMENT PRIMARY KEY, `Label` VARCHAR(255) NOT NULL, `Secret` VARCHAR(1000) NOT NULL);";

    public ColumnEncryptionIntegrationTests(MariaDbContainerFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task SecretRoundTripsButIsCiphertextAtRest()
    {
        ...
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "encrypt");
