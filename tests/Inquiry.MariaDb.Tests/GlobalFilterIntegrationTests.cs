...
/// <summary>
/// Global query filter against real MariaDB via the shared <see cref="GlobalFilterDoc"/> and
...
public sealed class GlobalFilterIntegrationTests
{
    ...
    public GlobalFilterIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task GlobalFilterHidesUnpublishedRowsFromEverySelect()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.GlobalFilterMySqlDdl, "gf");
