...
/// <summary>
/// Optimistic concurrency against real MariaDB via the shared <see cref="VersionedItem"/> catalog
...
public sealed class ConcurrencyIntegrationTests
{
    ...
    public ConcurrencyIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task SuccessfulUpdateBumpsVersion()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "conc");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "concreturn");
        var store = harness.GetRequiredService<VersionedItemStore>();
        var stale = await store.InsertAsync(new VersionedItem { Title = "v0" });
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "concthrow");
