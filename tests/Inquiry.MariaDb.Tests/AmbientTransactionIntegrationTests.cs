...
public sealed class AmbientTransactionIntegrationTests
{
    ...
    public AmbientTransactionIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task StoreCallInsideTransactionIsRolledBack()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "Ambient");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harnessA = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientA");
        await using var harnessB = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "AmbientB");
