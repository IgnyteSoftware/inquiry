...
/// real MariaDB: ordered results, offset page boundaries, keyset forward paging round-tripping
/// <c>NextCursor</c>, the null first page, <c>HasMore</c> at the end, and a multi-column keyset
/// tie-break. Keyset/offset selects use generator-synthesized <c>@__limit</c>/<c>@__offset</c>/cursor
/// parameters, so this exercises that the provider binds those against the live engine.
/// </summary>
[Collection(MariaDbCollection.Name)]
public sealed class PaginationIntegrationTests
{
    private readonly MariaDbContainerFixture _fixture;
    public PaginationIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;
    ...
    // Products carry an FK to Categories, so the referenced categories are created first.
    ...
    public async Task OrderByReturnsRowsInSortedOrder()
    {
        ...
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "pageorder");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "pageoffset");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "pagelast");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "pagepast");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "keyset");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "keysetexact");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "keysetmulti");
        await SeedAsync(harness);
        var products = harness.GetRequiredService<ProductStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "keysetempty");
        var products = harness.GetRequiredService<ProductStore>();
