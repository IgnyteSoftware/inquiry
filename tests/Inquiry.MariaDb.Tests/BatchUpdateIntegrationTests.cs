...
/// <summary>Batch update end-to-end against MariaDB: one UPDATE per row in a single command updates
...
public sealed class BatchUpdateIntegrationTests
{
    ...
    public BatchUpdateIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    private async Task<MariaDbTestHarness> SeedAsync()
    {
        var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "batchupd");
