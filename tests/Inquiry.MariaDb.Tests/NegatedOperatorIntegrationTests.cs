...
public sealed class NegatedOperatorIntegrationTests
{
    ...
    public NegatedOperatorIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = "CREATE TABLE `NegatedProduct` (`Id` BIGINT AUTO_INCREMENT PRIMARY KEY, `Name` VARCHAR(255) NOT NULL, `Qty` INT NOT NULL);";

    [SkippableFact]
    public async Task NotLikeExcludesPatternMatches()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "negated");
