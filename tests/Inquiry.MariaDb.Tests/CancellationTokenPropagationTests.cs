...
public sealed class CancellationTokenPropagationTests
{
    ...
    public async Task InquiryPipelineCancellationStopsInFlightOperationPromptly()
    {
        ...
        MariaDbTestHarness? harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancel");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelSelect");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelInsert");
        var store = harness.GetRequiredService<CustomerStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelStream");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelScalar");
        var inquiry = harness.GetRequiredService<IInquiry>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateAsync(_fixture.AdminConnectionString, "cancelTx");
        var inquiry = harness.GetRequiredService<IInquiry>();
    ...
    private static async Task ObserveAndDisposeAsync(Task execution, MariaDbTestHarness harness)
    {
        try
        {
