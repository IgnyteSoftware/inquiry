...
public sealed class GeneratedKeyUpsertTests
{
    ...
    public GeneratedKeyUpsertTests(MariaDbContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task NullKeyLetsDatabaseGenerateTheKey()
    {
        ...
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "gen_key_gen");
    ...
    {
        // #53: MariaDB does not update LAST_INSERT_ID() for an explicit-value insert, and the ON DUPLICATE
        ...
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "gen_key_explicit");
        var store = harness.GetRequiredService<GeneratedItemStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "gen_key_conc");
        var store = harness.GetRequiredService<GeneratedItemStore>();
