...
public sealed class DistinctIntegrationTests
{
    ...
    public DistinctIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = "CREATE TABLE `DistinctProduct` (`Id` BIGINT AUTO_INCREMENT PRIMARY KEY, `Name` VARCHAR(255) NOT NULL, `Category` VARCHAR(255) NOT NULL);";

    [SkippableFact]
    public async Task DistinctSelectAllDeduplicatesRows()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "distinct");
