...
/// <summary>
/// End-to-end <c>[InquiryWhere(JsonPath = ...)]</c> behaviour against real MariaDB: a criterion filters
...
public sealed class JsonPathPredicateIntegrationTests
{
    ...
    public JsonPathPredicateIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    private async Task<(MariaDbTestHarness Harness, JsonPathDocStore Store)> SeedAsync()
    {
        var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.JsonPathMySqlDdl, "jsonpath");
