...
{
    private const string Ddl = "CREATE TABLE `SeqDoc` (`Id` CHAR(36) NOT NULL PRIMARY KEY, `Title` VARCHAR(255) NOT NULL);";

    private readonly MariaDbContainerFixture _fixture;
    public SequentialGuidIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task UnsetKeyIsAssignedAndRoundTrips()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "seqguid");
