...
public sealed class GuidKeyUpsertTests
{
    ...
    public GuidKeyUpsertTests(MariaDbContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task NullKeyLetsDatabaseGenerateTheGuid()
    {
        ...
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "guid_gen");
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "guid_insert");
        var store = harness.GetRequiredService<GuidItemStore>();
    ...
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "guid_conc");
        var store = harness.GetRequiredService<GuidItemStore>();
