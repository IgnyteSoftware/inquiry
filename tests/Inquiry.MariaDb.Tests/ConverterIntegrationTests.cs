...
/// <summary>
/// Value converters against real MariaDB via the shared <see cref="JsonDoc"/> catalog entity: a custom value
...
public sealed class ConverterIntegrationTests
{
    ...
    public ConverterIntegrationTests(MariaDbContainerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task ConverterAndJsonColumnsRoundTrip()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await MariaDbTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.MySqlDdl, "conv");
