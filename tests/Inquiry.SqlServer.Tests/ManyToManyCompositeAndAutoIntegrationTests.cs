...
/// <summary>
/// End-to-end coverage for the two many-to-many extensions in #80 against real SqlServer: a related entity
...
public sealed class ManyToManyCompositeAndAutoIntegrationTests
{
    ...
    public ManyToManyCompositeAndAutoIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;
    private static async Task<(SqlServerTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)>
        CreateGridHarnessAsync(string adminConnectionString, string ddl, string name)
    {
        ...
        var harness = await SqlServerTestHarness.CreateFromDdlAsync(adminConnectionString, ddl, name,
            configureServices: s =>
            {
    ...
    private Task<SqlServerTestHarness> CompositeHarnessAsync()
        => SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.CompositeManyToManySqlServerDdl, "M2MComposite");

    private Task<SqlServerTestHarness> AutoHarnessAsync()
        => SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.AutoJunctionSqlServerDdl, "M2MAuto");
    ...
    public async Task CompositeKeyEagerLoadCostsOneRoundTrip()
    {
        ...
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.CompositeManyToManySqlServerDdl, "M2MCompositeGrid");
    ...
    public async Task CompositeKeyAllEagerCostsOneRoundTrip()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.CompositeManyToManySqlServerDdl, "M2MCompositeAllGrid");
        await using var _ = harness;
    ...
    public async Task AutoJunctionEagerLoadCostsOneRoundTrip()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        var (harness, probe, recorder) = await CreateGridHarnessAsync(
            _fixture.AdminConnectionString, FeatureSchema.AutoJunctionSqlServerDdl, "M2MAutoGrid");
        await using var _ = harness;
