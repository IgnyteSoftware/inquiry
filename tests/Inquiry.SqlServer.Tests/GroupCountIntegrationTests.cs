...
public sealed class GroupCountIntegrationTests
{
    ...
    public GroupCountIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = "CREATE TABLE [GroupItem] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [Category] NVARCHAR(MAX) NOT NULL, [Priority] INT NOT NULL);";

    [SkippableFact]
    public async Task CountByCategoryReturnsGroupedCounts()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "groupcount");
