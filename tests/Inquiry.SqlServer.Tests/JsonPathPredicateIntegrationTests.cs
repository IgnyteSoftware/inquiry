...
/// <summary>
/// End-to-end <c>[InquiryWhere(JsonPath = ...)]</c> behaviour against real SQL Server: a criterion filters
/// inside a JSON text column via the dialect's JSON_VALUE, supports nested paths, and AND-composes with an
/// ordinary criterion.
/// </summary>
...
public sealed class JsonPathPredicateIntegrationTests
{
    ...
    public JsonPathPredicateIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;

    private async Task<(SqlServerTestHarness Harness, JsonPathDocStore Store)> SeedAsync()
    {
        var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.JsonPathSqlServerDdl, "jsonpath");
