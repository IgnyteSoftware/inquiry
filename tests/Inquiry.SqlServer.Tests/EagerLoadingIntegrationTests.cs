...
public sealed class EagerLoadingIntegrationTests
{
    ...
    public EagerLoadingIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;
    ...
    private const string OrphanTerritoryDdl = """
        ...
        BEGIN
            CREATE TABLE [Region] ([RegionID] INT NOT NULL PRIMARY KEY, [RegionDescription] NVARCHAR(MAX) NOT NULL);
        END;

        IF OBJECT_ID(N'Territories', N'U') IS NULL
        BEGIN
            CREATE TABLE [Territories] ([TerritoryID] NVARCHAR(40) NOT NULL PRIMARY KEY, [TerritoryDescription] NVARCHAR(60) NOT NULL, [RegionID] INT NOT NULL);
        END;
        """;
    ...
    public async Task SelectOneByKeyEagerLoadsChildCollection()
    {
        ...
        await using var harness = await SqlServerTestHarness.CreateAsync(_fixture.AdminConnectionString, "Eager");
    ...
    public async Task SelectByKeyEagerLeavesParentNullForOrphanForeignKey()
    {
        ...
        await using var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, OrphanTerritoryDdl, "Eager");
        var territoryStore = harness.GetRequiredService<TerritoryStore>();
    ...
    private async Task<(SqlServerTestHarness Harness, BatchExecutionProbe Probe, RecordingCommandInterceptor Recorder)> CreateGridHarnessAsync()
    {
        ...
        var harness = await SqlServerTestHarness.CreateFromDdlAsync(
            _fixture.AdminConnectionString, NorthwindSchema.SqlServerDdl, "EagerGrid",
