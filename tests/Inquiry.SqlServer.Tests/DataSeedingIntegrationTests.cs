...
public sealed class DataSeedingIntegrationTests
{
    ...
    public DataSeedingIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;
    ...
    public async Task SeederPopulatesOnceAndIsIdempotentWithGuard()
    {
        ...
        const string ddl = "CREATE TABLE [SeededItem] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [Name] NVARCHAR(MAX) NOT NULL)";
        await using var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, ddl, "Seed");
