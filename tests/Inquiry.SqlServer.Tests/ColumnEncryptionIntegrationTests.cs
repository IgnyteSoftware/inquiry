...
public partial class SecretRecordStore : InquiryStore<SecretRecord>
{
    ...
    public partial Task<SecretRecord?> ByIdAsync(int id, CancellationToken cancellationToken = default);
}
...
public sealed class ColumnEncryptionIntegrationTests
{
    ...
    private const string Ddl =
        "CREATE TABLE [SecretRecord] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [Label] NVARCHAR(MAX) NOT NULL, [Secret] NVARCHAR(MAX) NOT NULL);";

    public ColumnEncryptionIntegrationTests(SqlServerContainerFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task SecretRoundTripsButIsCiphertextAtRest()
    {
        ...
        await using var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "encrypt");
