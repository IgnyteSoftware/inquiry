...
/// <summary>
/// W8 soft delete against real SQL Server via the shared <see cref="SoftItem"/> catalog entity: a soft
...
public sealed class SoftDeleteIntegrationTests
{
    ...
    public SoftDeleteIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;

    private async Task<(SqlServerTestHarness Harness, SoftItemStore Store, long Id)> SeedOneAsync()
    {
        var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, FeatureSchema.SqlServerDdl, "soft");
