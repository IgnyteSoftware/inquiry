...
public sealed class DistinctIntegrationTests
{
    ...
    public DistinctIntegrationTests(SqlServerContainerFixture fixture) => _fixture = fixture;

    private const string Ddl = "CREATE TABLE [DistinctProduct] ([Id] INT IDENTITY(1,1) PRIMARY KEY, [Name] NVARCHAR(MAX) NOT NULL, [Category] NVARCHAR(MAX) NOT NULL);";

    [SkippableFact]
    public async Task DistinctSelectAllDeduplicatesRows()
    {
        Skip.IfNot(_fixture.IsAvailable, _fixture.SkipReason);
        await using var harness = await SqlServerTestHarness.CreateFromDdlAsync(_fixture.AdminConnectionString, Ddl, "distinct");
