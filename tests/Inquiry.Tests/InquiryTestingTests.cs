...
public sealed class InquiryTestingTests
{
    ...
    [Fact]
    public async Task SandboxReportsUserExceptionAndCleanupFailuresWhenRollbackAndDisposalFail()
    {
        ...
        await using (services.ConfigureAwait(false))
        {
            ...
            var actual = await Assert.ThrowsAsync<AggregateException>(() => sandbox.RunAsync((context, cancellationToken) =>
            {
                context.Services.GetRequiredService<FailingScopeDisposable>();
                throw expected;
            }));

            Assert.Same(expected, actual.InnerExceptions[0]);
            Assert.Equal(
                new[] { "rollback failure", "transaction disposal failure", "scope disposal failure" },
                actual.InnerExceptions.Skip(1).Select(exception => exception.Message));
        }
    }

    [Fact]
    public async Task SandboxPreservesUserExceptionWhenCleanupSucceeds()
    {
        var services = new ServiceCollection()
            .AddScoped(_ => CreateSuccessfulSandboxInquiry())
            .BuildServiceProvider();
        await using (services.ConfigureAwait(false))
        {
            var sandbox = new InquirySandbox(services);
            ...
            var actual = await Assert.ThrowsAsync<InvalidOperationException>(
                () => sandbox.RunAsync((_, _) => throw expected));
    ...
    [Fact]
    public async Task FixtureReleasesKeeperConnectionWhenProviderDisposalFails()
    {
        var fixture = await SqliteInquiryFixture.CreateAsync(
            services => services.AddSingleton<FailingScopeDisposable>());
        fixture.Services.GetRequiredService<FailingScopeDisposable>();
        await fixture.ExecuteDdlAsync("CREATE TABLE Items (Id INTEGER PRIMARY KEY)");
        var connectionString = fixture.ConnectionString;

        await Assert.ThrowsAsync<InvalidOperationException>(async () => await fixture.DisposeAsync());

        // The keeper connection is the only thing keeping the shared-cache in-memory database
        // alive, so a released keeper means a reconnect sees a brand new, empty database.
        await using var probe = new SqliteConnection(connectionString);
        await probe.OpenAsync();
        var command = probe.CreateCommand();
        await using (command.ConfigureAwait(false))
        {
            command.CommandText = "SELECT COUNT(*) FROM Items";
            var missing = await Assert.ThrowsAsync<SqliteException>(() => command.ExecuteScalarAsync());
            Assert.Contains("no such table", missing.Message, StringComparison.OrdinalIgnoreCase);
        }
    }
