...
{
    private const string EntityAttributeNamespace = "Inquiry.Entities";
    private const string StoreAttributeNamespace = "Inquiry.Stores";
    private const string StoreNamespace = "Inquiry.Stores";
    ...
    private static Dictionary<string, EntityModel> DiscoverEntities(SourceProductionContext context, Compilation compilation, ImmutableArray<ClassDeclarationSyntax> candidates)
    {
        ...
        foreach (var classDeclaration in candidates)
        {
            ...
            foreach (var property in entitySymbol.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                var mappedColumnAttribute = keyAttribute ?? GetAttribute(property, "InquiryColumnAttribute");
                var foreignKeyAttribute = GetAttribute(property, "InquiryForeignKeyAttribute");
    ...
    private static EntityRegistrationModel GenerateEntityMetadata(SourceProductionContext context, EntityModel entity)
    {
        ...
        source.AppendLine("{");
        source.AppendLine($"    public string TableName => \"{Escape(entity.TableName)}\";");
        source.AppendLine();
        source.AppendLine($"    public string? Schema => {Literal(entity.Schema)};");
        source.AppendLine();
        ...
        {
            source.AppendLine($"            new global::Inquiry.Sql.InquirySqlColumn(\"{Escape(column.PropertyName)}\", \"{Escape(column.ColumnName)}\", isKey: {BooleanLiteral(column.IsKey)}, isGenerated: {BooleanLiteral(column.IsGenerated)}),");
        }
        ...
        if (entity.Columns.Any(static c => c.ForeignKey is not null))
        {
            ...
            {
                source.AppendLine($"            new global::Inquiry.Entities.InquiryForeignKey(\"{Escape(column.PropertyName)}\", \"{Escape(column.ColumnName)}\", \"{Escape(column.ForeignKey!.ReferencedTable)}\", \"{Escape(column.ForeignKey.ReferencedColumn)}\"),");
            }
    ...
    private static void GenerateMethod(StringBuilder source, StoreMethodModel method, EntityModel entity)
    {
        ...
        switch (method.Operation)
        {
            case StoreOperation.SelectAll:
                ...
                source.AppendLine("    {");
                source.AppendLine($"        return _inquiry.QueryAsync<{entity.Symbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}>(new global::Inquiry.Commands.InquiryCommandDefinition(_sqlStatements.SelectAll), {cancellation});");
                source.AppendLine("    }");
                break;
            ...
            case StoreOperation.SelectByField:
                ...
                source.AppendLine($"            _sqlStatements.SelectByField(new global::Inquiry.Sql.InquirySqlColumn(\"{Escape(method.FieldColumn!.PropertyName)}\", \"{Escape(method.FieldColumn.ColumnName)}\", isKey: {BooleanLiteral(method.FieldColumn.IsKey)}, isGenerated: {BooleanLiteral(method.FieldColumn.IsGenerated)})),");
    ...
    private static bool TryGetStoreEntityType(INamedTypeSymbol symbol, out ITypeSymbol entityType)
    {
        for (var current = symbol.BaseType; current is not null; current = current.BaseType)
        {
            if (current is { IsGenericType: true } &&
                current.ContainingNamespace.ToDisplayString() == StoreNamespace &&
    ...
    private static AttributeData? GetAttribute(ISymbol symbol, string shortName)
    {
        return symbol.GetAttributes().FirstOrDefault(a =>
            ...
            a.AttributeClass.ContainingNamespace.ToDisplayString() == EntityAttributeNamespace);
    }

    private static bool IsStoreAttribute(AttributeData attribute)
    {
        return attribute.AttributeClass?.ContainingNamespace?.ToDisplayString() == StoreAttributeNamespace;
    }
    ...
    private static string GetMappedColumnName(AttributeData? mappedColumnAttribute, AttributeData? foreignKeyAttribute, string propertyName)
    {
        ...
        {
            return GetConstructorString(mappedColumnAttribute) ?? propertyName;
        }
    ...
    private static string GetGeneratedEntityMetadataTypeName(EntityRegistrationModel registration)
    {
        if (registration.EntityType.ContainingNamespace.IsGlobalNamespace)
        {
            return "global::" + registration.MetadataTypeName;
        }

        return "global::" + registration.EntityType.ContainingNamespace.ToDisplayString() + "." + registration.MetadataTypeName;
    }
}
