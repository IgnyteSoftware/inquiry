...
internal static class StoreProcessor
{
        ...
        ImmutableArray<DiagnosticData>.Builder diagnostics)
    {
        ...
        if (operation == StoreOperation.GroupCount)
        {
            ...
            if (TryGetSelectElementType(method.ReturnType, out var gcElement, out _) && IsGroupCountElement(gcElement))
            {
    ...
    private static bool HasSupportedReturnType(StoreOperation operation, ITypeSymbol returnType, ITypeSymbol entityType, bool returnsEntity, AttributeData attribute, bool hasInputOutputParameter = false)
    {
        return operation switch
        {
            ...
            StoreOperation.GroupCount =>
                TryGetSelectElementType(returnType, out var groupElement, out var isGroupList) && isGroupList &&
                IsGroupCountElement(groupElement),
    ...
    /// </summary>
    /// <summary>True when the list element of a GroupCount method is <c>Inquiry.GroupCount&lt;TKey&gt;</c>.</summary>
    /// <remarks>
    /// The emitter materializes the key from this type argument, so any other element type leaves it
    /// with nothing to emit; requiring the shape here turns that into an INQ005 instead.
    /// </remarks>
    private static bool IsGroupCountElement(INamedTypeSymbol element)
        => element.IsGenericType &&
            element.TypeArguments.Length == 1 &&
            element.ConstructedFrom.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) == "global::Inquiry.GroupCount<TKey>";
        ...
        ISet<string>? invalidEntityNames = null)
    {
        ...
        if (needsDeleteAll)
        {
            ...
            AppendSql("_sqlDeleteAll", hasSoftDelete ? sqlBuilder.BuildSoftDeleteAllByKeysSql(ctx) : sqlBuilder.BuildDeleteAllByKeysSql(ctx));
            if (sqlBuilder.UsesArrayBindingForBatchMutations)
            {
                AppendSql("_sqlDeleteAllItem", hasSoftDelete ? sqlBuilder.BuildSoftDeleteByKeySql(ctx) : sqlBuilder.BuildDeleteByKeySql(ctx));
            }
        }
        ...
        string? baseSelectField)
    {
        ...
        switch (method.Operation)
        {
            ...
            case StoreOperation.DeleteAll:
                Add(sqlBuilder.UsesArrayBindingForBatchMutations
                    ? "array-bound command"
                    : "collection command",
                    sqlBuilder.UsesArrayBindingForBatchMutations ? "_sqlDeleteAllItem" : "_sqlDeleteAll");
                break;
    ...
    private static bool TryValidateForEmit(SourceProductionContext context, StoreMethodData method, EntityData entity, IReadOnlyDictionary<string, EntityData> relationChildEntities, IReadOnlyDictionary<string, EntityData> relationJunctionEntities, SqlBuilder sqlBuilder, out IReadOnlyList<ColumnData> fieldColumns, out ResolvedPredicatePlan? predicatePlan, out ResolvedSelectPlan? selectPlan)
    {
        ...
        // The ORDER BY / GROUP BY column of a top-by-order or group-count method is resolved
        // unguarded at emit time, so an unmapped name has to be rejected here first.
        if (method.Operation == StoreOperation.SelectTopByOrder && FindColumn(entity, method.TopByOrderColumn!) is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(InquiryDiagnosticDescriptors.UnknownField, method.Location?.ToLocation(), method.Name, method.TopByOrderColumn));
            return false;
        }

        if (method.Operation == StoreOperation.GroupCount && FindColumn(entity, method.GroupCountColumn!) is null)
        {
            context.ReportDiagnostic(Diagnostic.Create(InquiryDiagnosticDescriptors.UnknownField, method.Location?.ToLocation(), method.Name, method.GroupCountColumn));
            return false;
        }
