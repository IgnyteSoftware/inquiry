...
internal static class EntityProcessor
{
    ...
    private static EntityData ExtractCore(INamedTypeSymbol entitySymbol, Compilation compilation, bool isView, CancellationToken cancellationToken)
    {
        ...
        RemoveDuplicateColumns(columns, entitySymbol.Name, location, diagnostics);
        var relations = DiscoverRelations(entitySymbol, cancellationToken);
        var indexes = DiscoverIndexes(entitySymbol, columns, diagnostics);
    ...
    /// <summary>
    /// Reports INQ002 for every column name claimed by more than one property and drops all but the
    /// first claimant.
    /// </summary>
    /// <remarks>
    /// The drop is deliberately independent of the diagnostic: a consumer can downgrade or suppress
    /// INQ002 in .editorconfig, and keeping both properties would then emit an INSERT/SELECT list that
    /// repeats the identifier — SQL the provider rejects at runtime.
    /// </remarks>
    private static void RemoveDuplicateColumns(
        ...
        ImmutableArray<DiagnosticData>.Builder diagnostics)
    {
        ...
        for (var i = 0; i < columns.Count; i++)
        {
            ...
            diagnostics.Add(DiagnosticData.Create(
                InquiryDiagnosticDescriptors.DuplicateColumn, location, entityName, columns[i].ColumnName));
            columns.RemoveAt(i--);
        }
    }
    ...
    private static ImmutableArray<IndexData>.Builder DiscoverIndexes(
        ...
        ImmutableArray<DiagnosticData>.Builder diagnostics)
    {
        ...
        foreach (var attribute in entity.GetAttributes().Where(static a => a.AttributeClass?.ToDisplayString() == "Inquiry.Entities.InquiryIndexAttribute"))
        {
            ...
            // An unresolved key or Include used to collapse to "" and reach the DDL as a blank
            // identifier. Report it and drop the whole index (fail-closed): a partially resolved index
            // covers different columns than the author asked for, which is worse than no index.
            var unresolved = logicalKeys.Concat(logicalIncludes)
                .Where(p => !byProperty.ContainsKey(p))
                .ToImmutableArray();
            if (unresolved.Length > 0)
            {
                ...
                {
                    diagnostics.Add(DiagnosticData.Create(
                        InquiryDiagnosticDescriptors.IndexPropertyNotMapped, attributeLocation, entity.Name, name));
                }
                continue;
            }

            var keys = logicalKeys.Select(p => byProperty[p].ColumnName).ToImmutableArray();
            var includes = logicalIncludes.Select(p => byProperty[p].ColumnName).ToImmutableArray();
            result.Add(new IndexData(null, string.Empty, new EquatableArray<string>(keys), new EquatableArray<string>(includes),
                ...
                LocationData.From(attributeLocation))
            {
