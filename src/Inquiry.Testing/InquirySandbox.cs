...
public sealed class InquirySandbox
{
    ...
    /// </summary>
    /// <remarks>
    /// If the operation and the rollback/disposal both fail, an <see cref="AggregateException"/> is
    /// thrown whose first inner exception is the operation's failure.
    /// </remarks>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (primaryFailure is not null)
        {
            ...
            // Matches the runtime's cleanup contract: the primary failure stays the first inner
            // exception so callers can still find it, with cleanup failures appended in order.
            var failures = new List<Exception>(cleanupFailures.Count + 1) { primaryFailure.SourceException };
            failures.AddRange(cleanupFailures);
            throw new AggregateException(
                "The Inquiry sandbox operation failed and its rollback or disposal also failed.",
                failures);
        }
