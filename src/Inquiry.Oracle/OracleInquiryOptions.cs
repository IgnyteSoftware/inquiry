...
{
    /// <summary>
    /// Gets or sets the target Oracle-compatible engine. Defaults to
    /// <see cref="OracleCompatibility.None"/> (no open-time retry).
    /// </summary>
    public OracleCompatibility Compatibility { get; set; } = OracleCompatibility.None;
    ...
    /// Oracle TNS descriptors with <c>ADDRESS_LIST</c>/<c>FAILOVER=on</c> remain available when
