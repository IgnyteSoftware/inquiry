...
{
    private MariaDbBenchmarkDatabase _db = null!;
    ...
    public void GlobalSetup()
        => _db = MariaDbBenchmarkDatabase.CreateAsync(Rows).GetAwaiter().GetResult();
    ...
    [BenchmarkCategory("SelectByKey"), Benchmark]
    public async Task<MariaDbEfShipper?> SelectByKey_EfCore()
    {
        await using var ctx = await _db.DbContextFactory.CreateDbContextAsync();
    ...
    {
        await using var ctx = await _db.DbContextFactory.CreateDbContextAsync();
        ctx.Shippers.Add(new MariaDbEfShipper { CompanyName = "Bench Shipper", Phone = "555-0000" });
