//
// Class	:	CustomerCustomerDemos.cs
...
{
	public class CustomerCustomerDemos :Collection<CustomerCustomerDemo>, IEnumerable<CustomerCustomerDemo>, IComparer
	{
		 ...
		 public class Enumerator : IEnumerator
		 {
			...
			private CustomerCustomerDemos t;

			public Enumerator (CustomerCustomerDemos t)
			{
				this.t = t;
			}
		...
		public void Sort(string sortingExpression)
		{
			...
			CustomerCustomerDemo[] elements =new CustomerCustomerDemo[this.Items.Count];
			this.Items.CopyTo(elements,0);
			Array.Sort(elements,this);
			//clear old order and copy new order here
			this.Items.Clear();
			foreach(CustomerCustomerDemo item in elements)
				this.Items.Add(item);
		}
		...
		{
			return new CustomerCustomerDemos.Enumerator(this);
		}
		...
		public int Compare( object o1,  object o2)
		{
			...
			for ( int i = 0 ; i < sortKeys.Length; i++ )
			{
				...
				if ( sortKeys[i].FieldInfo is not null )
				{
		...
		{
			if (this.Items.Count is 0) return;
			...
			for ( int i = 0; i < props.Length; i++ )
			{
				...
				if ( sortKeys[i].FieldInfo is null )
				{
