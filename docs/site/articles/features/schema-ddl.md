...
- `ProviderArtifactsValidationSql` - a read-only query that returns one row per invalid or unverifiable artifact.
- `Ddl` - the compatible full baseline: provider artifacts first, followed by table/index DDL.

SQL Server uses these constants for schema-qualified TVP types. Applying artifacts is an explicit deployment step; generated commands never create or discover types at runtime. Validation returns `ArtifactName`, `ExpectedElementSignature`, `Status`, and `Details`. `Status` is `missing`, `mismatched`, or `metadata-invisible`; a valid deployment returns no rows. A mismatch includes the exact system type/facets/nullability, column count/name, alias type, identity/computed flags, collation, indexes, keys, defaults, and checks.
...
- **SQL Server collection artifacts**: deterministic v2 `CREATE TYPE [schema].[Inquiry_Tvp_<sha256>] AS TABLE ([Value] <exact physical signature> NULL|NOT NULL)` statements for supported positive `IN` and `[InquiryDeleteAll]` usages. The hash covers the provider type/facets and nullability; the schema qualifies the object. Negative `NOT IN` expansion does not require a TVP artifact.
