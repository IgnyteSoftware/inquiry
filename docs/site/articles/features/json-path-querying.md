...
- **The path must be a well-formed dotted object path** — `$` then one or more `.segment`, where each segment is an **unquoted identifier**: a letter or `_`, then letters/digits/`_` (so `$.address.city` and `$.line_1` are fine). Anything else — a bare `$`, a trailing/empty segment, an array index (`$.a[0]`), a hyphenated or digit-leading key (`$.first-name`, `$.0`), a quoted or space-bearing key — is rejected with **`INQ060`**. This is the cross-dialect subset that needs no quoting in any engine's JSON-path syntax (SQL Server / MySQL / Oracle require quoting for hyphenated or digit-leading keys, which v1 doesn't emit); such keys are a future addition rather than something silently mis-handled.
