...
| Auto-key | `SERIAL` / `BIGSERIAL PRIMARY KEY` (64-bit keys use `BIGSERIAL`) |
| Upsert | `INSERT … ON CONFLICT (pk) DO UPDATE SET …` |
| Insert-returning | `INSERT … RETURNING <columns>` (the explicit quoted column list, not `*`) |
...
- **Date/time values:** `DateTime` maps to PostgreSQL `TIMESTAMP` (without time zone). Inquiry preserves its wall-clock ticks while binding it with `Kind = Unspecified`, and materialized values therefore have `DateTimeKind.Unspecified`; PostgreSQL stores microsecond precision and does not persist the original `Kind`. `DateTimeOffset` maps to `TIMESTAMPTZ` for instant semantics, which PostgreSQL normalizes to UTC without retaining the original offset. Npgsql accepts only zero-offset `DateTimeOffset` values for this type, so applications must explicitly normalize non-zero offsets before calling Inquiry.
- **Cloud retry (opt-in):** off by default (`Compatibility = None`). Set `PostgreSqlInquiryOptions.Compatibility` to `AuroraPostgreSql` (retries the `08xxx` connection family plus `57P01` on open) or `CockroachDb` (retries `40001` / `08006` / `57P01`); retry covers connection-open only, not command execution.
