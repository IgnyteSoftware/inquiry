...
- **Apples-to-apples performance with raw ADO.NET.** Generated readers use `CommandBehavior.SequentialAccess` and per-call struct materializers — on buffered CRUD workloads (select-all, point reads, insert, update, upsert), measured allocation matches hand-written ADO.NET. See the [benchmark suite](https://github.com/IgnyteSoftware/inquiry/tree/main/benchmarks) for workload-scoped evidence and methodology.
...
- **[Providers](articles/providers/sqlite.md)** — per-dialect notes for the 6 supported engines.
